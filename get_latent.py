...
# python predict.py pre-trained/formation-energy-per-atom.pth.tar data/Relaxation
...
def main():
    ...
    file = open("Latents.csv", mode='w')
    file.write("Name, Target, Values")
...
def validate(val_loader, model, criterion, normalizer, file, test=False):
    ...
    for i, (input, target, batch_cif_ids) in enumerate(val_loader):
        ...
        latent = model.get_last_hidden(*input_var)
        for output_i in range(len(output)):
            file.write("{},{},{}\n".format(batch_cif_ids[output_i], target_var[output_i].item(), ",".join([str(num) for num in latent[output_i].tolist()])))
